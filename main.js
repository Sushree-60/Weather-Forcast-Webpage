...
.then(data => {
    ...
    for(i = 0; i<7; i++){
        document.getElementById("day" + (i+1) + "Pressure").innerHTML = "Pressure: " + Number(data.list[i].main.pressure).toFixed();
    }

    for(i = 0; i<7; i++){
        document.getElementById("day" + (i+1) + "Humidity").innerHTML = "Humidity: " + Number(data.list[i].main.humidity).toFixed();
    }
    //------------------------------------------------------------
...
// .catch(err => alert("Plzz Enter the correct city"))
}
